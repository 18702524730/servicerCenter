<template>
	<div class="stepTwoFill" v-loading="loading" element-loading-text="拼命加载中">
		<!-- 主体信息 -->
		<div class="common_c main_body" v-if="!loading">
	      ...
	      <div v-if="apply_type==2" key="2">
		      <el-form-item label="申请人企业名称" prop="apply_name" class="apply_name">
		        <!-- <el-autocomplete size="small" v-model="ruleForm.apply_name" :fetch-suggestions="querySearch" :trigger-on-focus="false" @select="handleSelect" :maxlength="60" placeholder="请填写申请人企业名称，与企业营业执照信息保持一致"></el-autocomplete> -->
    ...
    <!-- 开票信息 -->
		<div class="common_c invoice_info" v-if="!loading">
	      ...
	      <template v-if="apply_type==2 && needInvoice">
		      <el-form-item label="开票类型" style="margin-bottom:10px;" required>
			      <el-radio-group v-model="invoice_type" @change="invoiceTypeChange()">
			        <el-radio class="radio" :label="1">增值税普通发票</el-radio>
			  			<el-radio class="radio" :label="2"> 增值税专用发票</el-radio>
		  			</el-radio-group>
		      </el-form-item>
		      ...
		      <div v-if="invoice_type==2">
			      ...
			      <el-form-item label="纳税人识别号" prop="invoice_code" required>
			        <el-input size="small" v-model="invoiceForm.invoice_code" :maxlength="20" placeholder="请填写纳税人识别号"></el-input>
			      </el-form-item>
			      <el-form-item label="注册电话" prop="regist_phone" required>
		          ...
		          <el-form-item class="form-ipt-sm" prop="province_id" style="display:inline-block;width:150px;">
		            <el-select v-model="invoiceForm.province_id" clearable filterable default-first-option placeholder="请选择省" @change="changeProvince('invoiceForm', invoiceForm.province_id, 'invoice_')">
		              <SebeOption v-for="item in provinceData" :label="item.name" :filterLabel="item.name+(item.areaPinYin || '')+(item.areaPinYinAbbr || '')" :value="item.area_id" :key="item.area_id"></SebeOption>
		            </el-select>
		          </el-form-item>
		          <el-form-item class="form-ipt-sm" prop="city_id" style="display:inline-block;width:150px;">
		            <el-select v-model="invoiceForm.city_id" clearable filterable default-first-option placeholder="请选择市" @change="changeCity('invoiceForm', invoiceForm.city_id, 'invoice_')">
		          ...
		          <el-form-item class="form-ipt-sm form-ipt-last" prop="area_id" style="display:inline-block;width:150px;">
		            <el-select v-model="invoiceForm.area_id" clearable filterable default-first-option placeholder="请选择区">
		          ...
		          <el-form-item prop="address" style="margin-top:20px;margin-bottom:0;">
		            <el-input v-model="invoiceForm.address" :maxlength="80" placeholder="请填写企业注册地址，与企业登记证件保持一致" auto-complete="off" class="form-ipt"></el-input>
		          </el-form-item>
		          <a class="fill_address" @click="fillAddress('invoiceForm', 'ruleForm', '', '', 'invoice_')">复用主体地址</a>
		        </el-form-item>
			      <el-form-item label="开户行名称" prop="account_bank" required>
...
<style lang='less'>
	.stepTwoFill{padding:60px 90px;background-color:#fff;min-height: 500px;
		// 字体淡黄色
		.cy{color: #F5A623;}
		...
		.common_c{
			...
			//模板 上下两部分共用
			.common_c_template{
				float: right;width:320px; margin-right: 170px;margin-top: -44px;background: #FFFFFF;padding-bottom:2000px;margin-bottom:-2000px;
		...
		.confirm_btn{width:327px;margin:40px auto 0;
	...
	.apply_name{
		.el-autocomplete{width:470px;}
	}
	.el-select-dropdown{width:140px;}
	// .el-select-dropdown .el-scrollbar{width:470px;}
	// .form-ipt-sm .el-select-dropdown .el-scrollbar{width:148px;}
</style>
