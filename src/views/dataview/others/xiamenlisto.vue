      ...
      <table class="result_type">
        <thead>
          <tr class="thead">
            ...
            <th class="comWidth" v-show="ziduan.indexOf(7)>-1">代理专员</th>
            ...
            <th class="comWidth" v-show="ziduan.indexOf(13)>-1">收款日期</th>
            <th class="comWidth" v-show="ziduan.indexOf(14)>-1">申报日期</th>
            <th class="comWidth" v-show="ziduan.indexOf(15)>-1">准予注册收文日期</th>
            <th class="comWidth" v-show="ziduan.indexOf(16)>-1">申报员</th>
            <th class="comWidth" v-show="ziduan.indexOf(17)>-1">申请号</th>
            <th class="comWidth" v-show="ziduan.indexOf(18)>-1">杭州拾贝收-代报费</th>
            <th class="comWidth" v-show="ziduan.indexOf(19)>-1">官费发票号</th>
            <th class="comWidth" v-show="ziduan.indexOf(20)>-1">服务费发票号</th>

            <th class="comWidth" v-show="ziduan.indexOf(21)>-1">收件人</th>
            <th class="comWidth" v-show="ziduan.indexOf(22)>-1">收件电话</th>
            <th class="comWidth" v-show="ziduan.indexOf(23)>-1">收到受理通知书</th>
            <th class="comWidth" v-show="ziduan.indexOf(24)>-1">补正通知书</th>
            <th class="comWidth" v-show="ziduan.indexOf(25)>-1">不予核准通知书收文日期</th>
            <th class="comWidth" v-show="ziduan.indexOf(26)>-1">注册证</th>
        ...
        <tbody class="tr-color">
          <tr v-for="(item, index) in dataList" v-show="!loading && dataList.length" :key="index">
            <td>{{item.seqNum}}</td>
            <td v-show="ziduan.indexOf(1)>-1">{{item.submitDate}}</td>
            ...
            <td v-show="ziduan.indexOf(7)>-1">{{item.agentCommissioner}}</td>
            <td v-show="ziduan.indexOf(8)>-1">{{item.reportType}}</td>
            ...
            <td v-show="ziduan.indexOf(13)>-1">{{item.collectionDate}}</td>
            <td v-show="ziduan.indexOf(14)>-1">{{item.reportDate}}</td>
            <td v-show="ziduan.indexOf(15)>-1">{{item.permitRegisterDate|dateFormat}}</td>
            <td v-show="ziduan.indexOf(16)>-1">{{item.reporter}}</td>
            <td v-show="ziduan.indexOf(17)>-1">{{item.applicationNo}}</td>
            <td v-show="ziduan.indexOf(18)>-1">{{item.sebeCollection}}</td>
            <td v-show="ziduan.indexOf(19)>-1">{{item.officialFeesInvoiceNumber}}</td>
            <td v-show="ziduan.indexOf(20)>-1">{{item.serviceChargeInvoiceNumber}}</td>

            <td v-show="ziduan.indexOf(21)>-1">{{item.invoiceRevicer}}</td>
            <td v-show="ziduan.indexOf(22)>-1">{{item.receviePhone}}</td>
            <td v-show="ziduan.indexOf(23)>-1">{{item.getAcceptNotice}}</td>
            <td v-show="ziduan.indexOf(24)>-1">{{item.updateNotice}}</td>
            <td v-show="ziduan.indexOf(25)>-1">{{item.refuseCheckNoticeDate}}</td>
            <td v-show="ziduan.indexOf(26)>-1">{{item.registration}}</td>
...
<script>
  ...
  const delUrl = './api/deleteXiamenTrademarkOtherById';
  const listUrl = './api/selectXiamenTrademarkOther'
  // 检查标签是否支持该属性
  ...
  {key:13,label:'收款日期'},
	{key:14,label:'申报日期'},
	{key:15,label:'准予注册收文日期'},
	{key:16,label:'申报员'},
	{key:17,label:'申请号'},
	{key:18,label:'杭州拾贝收-代报费'},
	{key:19,label:'官费发票号'},
  {key:20,label:'服务费发票号'},
  
	{key:21,label:'收件人'},
  {key:22,label:'收件电话'},
  {key:23,label:'收到受理通知书'},
	{key:24,label:'补正通知书'},
  {key:25,label:'不予核准通知书收文日期'},
  {key:26,label:'注册证'},
  ...
  export default {
    ...
    mounted() {
      let that = this;
      let getlist = store.get('userdefaultxiaolist')||this.ziduan;
      this.ziduan = getlist;
    ...
    methods: {
      ...
      exportlist(){
        ...
        console.log(obj)
        window.open(this.rootUrl+'/api/exportXiamenTrademarkOther?'+$.param(obj)+'&access_token='+this.token)
      },
      setlist(){
        store.set('userdefaultxiaolist',this.ziduan)
      },
      ...
      toaddrecord() {
        this.$router.push({name:'xiamenaddo',params:{id:0}});
      },
      findOrderOriginAll() {
      ...
      view(item){
        this.$router.push({name:'xiamenaddo',params:{id:item.id}});
      },
      edit(item){
        this.$router.push({name:'xiamenaddo',params:{id:item.id}});
      },
