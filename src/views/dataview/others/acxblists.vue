<template>
  <div class="commonList clientServiceOrder">
      ...
      <div class="form_control"><el-input v-model="dataObj.ext20" placeholder="申请号"></el-input></div>
...
<script>
  ...
  export default {
    ...
    mounted() {
      let that = this;
      this.setzd()
      let getlist = store.get('acxbxlist')||this.ziduan;
      this.ziduan = getlist;
    ...
    methods: {
      ...
      setlist(){
        store.set('acxbxlist',this.ziduan)
      },
      ...
      toaddrecord() {
        this.$router.push({name:'acxbadds',params:{fid:0,id:0}});
      },
      findOrderOriginAll() {
      ...
      view(item){
        this.$router.push({name:'acxbadds',params:{fid:item.dataUniqueNumber,id:item.id}});
      },
      edit(item){
        this.$router.push({name:'acxbadds',params:{fid:item.dataUniqueNumber,id:item.id}});
      },
