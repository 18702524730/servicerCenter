      ...
      <div class="basic_category clearfix">
        ...
        <div class="itembasic">
          <el-form ref="form1" label-width="270px">
            <el-form-item :label="`${item.name}：`" v-for="(item,index) in addData" :key="index" :class="{'spec':item.textboxType==1}" v-if="caseSysNo==0&&item.ifFinanceEdit==0">
              
              <el-date-picker
                v-if="item.textboxType==1&&item.ifFinanceEdit==0"
                v-model="item.value"
                type="date"
                placeholder="选择日期时间">
              </el-date-picker>
              <el-input type="text" v-if="item.textboxType==0&&item.ifFinanceEdit==0" v-model="item.value" :placeholder="`请填写${item.name}`" ></el-input>
            ...
            <el-form-item :label="`${item.name}：`" v-for="(item,index) in addData" :key="index" :class="{'spec':item.textboxType==1}" v-if="caseSysNo!=0">
              <el-date-picker
                v-if="item.textboxType==1"
                v-model="item.value"
                type="date"
                :disabled="item.ifFinanceEdit==1"
                placeholder="选择日期时间">
              </el-date-picker>
              <el-input type="text" v-if="item.textboxType==0" :disabled="item.ifFinanceEdit==1" v-model="item.value" :placeholder="`请填写${item.name}`" ></el-input>
              <el-select v-model="item.value" v-if="item.textboxType==2">
                <el-option v-for="it in item.textboxDisplayArr" :key="it" :label="it" :value="it">{{it}}</el-option>
              </el-select>
            </el-form-item>
    ...
    <div v-show="now==2">
      <div class="basic_lists materialbox">
          ...
          <div class="upload_component" style="margin-bottom:20px;">
            <pasuploader @complete="setUploadedMaterial" @resetUploader="resetUploader" :fileName="standby8_name" :canPreview="false" :postAction="'iprp_servicer/api/uploadFile?offlineType=1210&offlineId='+caseSysNo+'&access_token='+token" :url="standby8" :valueName="'standby8'"  :size="10"></pasuploader>
