<template>
  <div class="commonList clientServiceOrder">
      ...
      <div class="form_control"><el-input v-model="dataObj.trademarkApplicationNo" placeholder="商标申请号"></el-input></div>
      ...
      <table class="result_type">
        <thead>
          <tr class="thead">
            ...
            <th class="minWidth" v-show="ziduan.indexOf(7)>-1">申请人</th>
            <th class="minWidth" v-show="ziduan.indexOf(8)>-1">服务商</th>
            <th class="minWidth" v-show="ziduan.indexOf(9)>-1">代理专员</th>
            <th class="comWidth" v-show="ziduan.indexOf(10)>-1">申报类别</th>

            <th class="comWidth" v-show="ziduan.indexOf(11)>-1">加项</th>
            <th class="maxWidth" v-show="ziduan.indexOf(12)>-1">官费</th>
            <th class="comWidth" v-show="ziduan.indexOf(13)>-1">服务费</th>
            <th class="comWidth" v-show="ziduan.indexOf(14)>-1">总金额</th>
            <th class="minWidth" v-show="ziduan.indexOf(15)>-1">收款日期</th>
            <th class="minWidth" v-show="ziduan.indexOf(16)>-1">申报日期</th>
            <th class="comWidth" v-show="ziduan.indexOf(17)>-1">申报员</th>
            <th class="minWidth" v-show="ziduan.indexOf(18)>-1">商标申请号</th>
            <th class="comWidth" v-show="ziduan.indexOf(19)>-1">上传申请号回执单</th>
            <th class="comWidth" v-show="ziduan.indexOf(20)>-1">核账人</th>

            <th class="comWidth" v-show="ziduan.indexOf(21)>-1">纳税人识别号</th>
            <th class="comWidth" v-show="ziduan.indexOf(22)>-1">官费发票号</th>
            <th class="comWidth" v-show="ziduan.indexOf(23)>-1">服务费发票号</th>
            <th class="comWidth" v-show="ziduan.indexOf(24)>-1">发票收件人</th>
            <th class="comWidth" v-show="ziduan.indexOf(25)>-1">收件人电话</th>
            <th class="comWidth" v-show="ziduan.indexOf(26)>-1">收件地址</th>
            <th class="comWidth" v-show="ziduan.indexOf(27)>-1">快递单号</th>
            <th class="comWidth" v-show="ziduan.indexOf(28)>-1">收到受理通知书</th>
            <th class="comWidth" v-show="ziduan.indexOf(29)>-1">驳回收文日期</th>
            <th class="comWidth" v-show="ziduan.indexOf(30)>-1">驳回短信通知</th>

            <th class="comWidth" v-show="ziduan.indexOf(31)>-1">是否驳回复审</th>
            <th class="comWidth" v-show="ziduan.indexOf(32)>-1">初审收文日期</th>
            <th class="comWidth" v-show="ziduan.indexOf(33)>-1">初审短信通知</th>
            <th class="comWidth" v-show="ziduan.indexOf(34)>-1">异议答辩收文</th>
            <th class="comWidth" v-show="ziduan.indexOf(35)>-1">是否异议答辩</th>
            <th class="comWidth" v-show="ziduan.indexOf(36)>-1">异议决定书收文</th>
            <th class="comWidth" v-show="ziduan.indexOf(37)>-1">商标证是否收文</th>
            <th class="comWidth" v-show="ziduan.indexOf(38)>-1">商标证短信通知</th>
            <th class="comWidth" v-show="ziduan.indexOf(39)>-1">商标转让需求</th>
            <th class="comWidth" v-show="ziduan.indexOf(40)>-1">收件人</th>

            <th class="comWidth" v-show="ziduan.indexOf(41)>-1">收件人电话</th>
            <th class="comWidth" v-show="ziduan.indexOf(42)>-1">收件人地址</th>
            <th class="comWidth" v-show="ziduan.indexOf(43)>-1">寄件快递单号</th>
            <th class="comWidth" v-show="ziduan.indexOf(44)>-1">寄件日期</th>
            <th class="comWidth" v-show="ziduan.indexOf(45)>-1">邮寄方式</th>
            <th class="comWidth" v-show="ziduan.indexOf(46)>-1">备注</th>
        ...
        <tbody class="tr-color">
          <tr v-for="(item, index) in dataList" v-show="!loading && dataList.length" :key="index">
            ...
            <td v-show="ziduan.indexOf(10)>-1">{{item.reportType}}</td>

            <td v-show="ziduan.indexOf(11)>-1">{{item.addition}}</td>
            ...
            <td v-show="ziduan.indexOf(15)>-1">{{item.collectionDate}}</td>
            <td v-show="ziduan.indexOf(16)>-1">{{item.reportDate}}</td>
            <td v-show="ziduan.indexOf(17)>-1">{{item.applicationNo}}</td>
            <td v-show="ziduan.indexOf(18)>-1">{{item.trademarkApplicationNo}}</td>
            <td v-show="ziduan.indexOf(19)>-1">{{item.uploadApplicationNo}}</td>
            <td v-show="ziduan.indexOf(20)>-1">{{item.checkMan}}</td>

            <td v-show="ziduan.indexOf(21)>-1">{{item.taxpayerNumber}}</td>
            <td v-show="ziduan.indexOf(22)>-1">{{item.officialFeesInvoiceNumber}}</td>
            <td v-show="ziduan.indexOf(23)>-1">{{item.serviceChargeInvoiceNumber}}</td>
            <td v-show="ziduan.indexOf(24)>-1">{{item.invoiceRevicer}}</td>
            <td v-show="ziduan.indexOf(25)>-1">{{item.receviePhone}}</td>
            <td v-show="ziduan.indexOf(26)>-1">{{item.recevieAddress}}</td>
            <td v-show="ziduan.indexOf(27)>-1">{{item.expressNumber}}</td>
            <td v-show="ziduan.indexOf(28)>-1">{{item.getAcceptNotice |dateFormat }}</td>
            <td v-show="ziduan.indexOf(29)>-1">{{item.rejectReceiptDate}}</td>
            <td v-show="ziduan.indexOf(30)>-1">{{item.rejectMessageNotice}}</td>

            <td v-show="ziduan.indexOf(31)>-1">{{item.whetherRejectReview}}</td>
            <td v-show="ziduan.indexOf(32)>-1">{{item.firstcheckReceiptDate |dateFormat}}</td>
            <td v-show="ziduan.indexOf(33)>-1">{{item.firstcheckMessageNotice |dateFormat }}</td>
            <td v-show="ziduan.indexOf(34)>-1">{{item.whetherDissentDefence}}</td>
            <td v-show="ziduan.indexOf(35)>-1">{{item.dissentDefenceReceipt}}</td>
            <td v-show="ziduan.indexOf(36)>-1">{{item.objectionDecisionReceipt |dateFormat}}</td>
            <td v-show="ziduan.indexOf(37)>-1">{{item.trademarkWhetherReceipt |dateFormat}}</td>
            <td v-show="ziduan.indexOf(38)>-1">{{item.trademarkMessageNotice}}</td>
            <td v-show="ziduan.indexOf(39)>-1">{{item.trademarkTransDemand}}</td>
            <td v-show="ziduan.indexOf(40)>-1">{{item.officialReceiver}}</td>

            <td v-show="ziduan.indexOf(41)>-1">{{item.receiverPhone}}</td>
            <td v-show="ziduan.indexOf(42)>-1">{{item.receiverAddress}}</td>
            <td v-show="ziduan.indexOf(43)>-1">{{item.mailExpressNumber}}</td>
            <td v-show="ziduan.indexOf(44)>-1">{{item.expressTime}}</td>
            <td v-show="ziduan.indexOf(45)>-1">{{item.emailType}}</td>
            <td v-show="ziduan.indexOf(46)>-1">{{item.remarks}}</td>
            <td class="onlyone">
...
<script>
  ...
  const delUrl = './api/deleteTrademarkRegisterylById';
  const listUrl = './api/selectTrademarkRegisteryl'
  // 检查标签是否支持该属性
  ...
  {key:34,label:'异议答辩收文'},
  {key:35,label:'是否异议答辩'},
	{key:36,label:'异议决定书收文'},
	{key:37,label:'商标证是否收文'},
  {key:38,label:'商标证短信通知'},
	{key:39,label:'商标转让需求'},
    {key:40,label:'收件人'},
  ...
  export default {
    ...
    mounted() {
      let that = this;
      let getlist = store.get('userdefaultofflinexlist')||this.ziduan;
      this.ziduan = getlist;
    ...
    methods: {
      ...
      exportlist(){
        ...
        console.log(obj)
        window.open(this.rootUrl+'/api/exportTrademarkRegisteryl?'+$.param(obj)+'&access_token='+this.token)
      },
      setlist(){
        store.set('userdefaultofflinexlist',this.ziduan)
      },
      ...
      toaddrecord() {
        this.$router.push({name:'dofflineaddx',params:{id:0}});
      },
      findOrderOriginAll() {
      ...
      view(item){
        this.$router.push({name:'dofflineaddx',params:{id:item.id}});
      },
      edit(item){
        this.$router.push({name:'dofflineaddx',params:{id:item.id}});
      },
