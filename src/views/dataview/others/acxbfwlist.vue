<template>
  <div class="commonList clientServiceOrder">
      ...
      <div class="form_control"><el-input v-model="dataObj.ext19" placeholder="申请号"></el-input></div>
...
<script>
  ...
  export default {
    ...
    mounted() {
      let that = this;
      this.setzd()
      let getlist = store.get('acxbfwlist')||this.ziduan;
      this.ziduan = getlist;
    ...
    methods: {
      ...
      setlist(){
        store.set('acxbfwlist',this.ziduan)
      },
      ...
      toaddrecord() {
        this.$router.push({name:'acxbfwadd',params:{fid:0,id:0}});
      },
      findOrderOriginAll() {
      ...
      findCustPlatformAll() {
        ...
        ret.tbName = 'A_Cxb_SP_Settle2016'
      ...
      view(item){
        this.$router.push({name:'acxbfwadd',params:{fid:item.dataUniqueNumber,id:item.id}});
      },
      edit(item){
        this.$router.push({name:'acxbfwadd',params:{fid:item.dataUniqueNumber,id:item.id}});
      },
