      ...
      <table class="result_type">
        <thead>
          <tr class="thead">
            ...
            <th class="minWidth" v-show="ziduan.indexOf(19)>-1">总金额</th>
            <th class="comWidth" v-show="ziduan.indexOf(20)>-1">下单时间</th>
            ...
            <th class="comWidth" v-show="ziduan.indexOf(37)>-1">发票收件人</th>
            ...
            <th class="comWidth" v-show="ziduan.indexOf(42)>-1">准予注册决定书收文日期</th>
            <th class="comWidth" v-show="ziduan.indexOf(43)>-1">不予注册决定书收文日期</th>
            <th class="comWidth" v-show="ziduan.indexOf(44)>-1">补正通知书收文</th>
            ...
            <th class="comWidth" v-show="ziduan.indexOf(65)>-1">收件人</th>
        ...
        <tbody class="tr-color">
          <tr v-for="(item, index) in dataList" v-show="!loading && dataList.length" :key="index">
            ...
            <td v-show="ziduan.indexOf(42)>-1">{{item.permissionRegisterReceipt|dateFormat}}</td>
            <td v-show="ziduan.indexOf(43)>-1">{{item.refuseRegisterDecisionDate|dateFormat}}</td>
            <td v-show="ziduan.indexOf(44)>-1">{{item.updateNoticeReceipt}}</td>
            <td v-show="ziduan.indexOf(45)>-1">{{item.acceptWhetherReceipt}}</td>
...
<script>
  ...
  const delUrl = './api/deleteDeclareybById';
  const listUrl = './api/selectDeclareyb'
  // 检查标签是否支持该属性
  ...
  {key:40,label:'备注'},
	...
	{key:41,label:'不予受理收文'},
	{key:42,label:'准予注册决定书收文日期'},
	{key:43,label:'不予注册决定书收文日期'},
	{key:44,label:'补正通知书收文'},
  {key:45,label:'受通是否收文'},
  {key:46,label:'受通是否上传'},
	{key:47,label:'受通短信通知'},
  ...
  export default {
    ...
    mounted() {
      let that = this;
      let getlist = store.get('userdefaultsebelist')||this.ziduan;
      this.ziduan = getlist;
    ...
    methods: {
      ...
      exportlist(){
        ...
        console.log(obj)
        window.open(this.rootUrl+'/api/exportDeclareyb?'+$.param(obj)+'&access_token='+this.token)
      },
      setlist(){
        store.set('userdefaultsebelist',this.ziduan)
      },
      ...
      toaddrecord() {
        this.$router.push({name:'bsebeadd',params:{id:0}});
      },
      findOrderOriginAll() {
      ...
      view(item){
        this.$router.push({name:'bsebeadd',params:{id:item.id}});
      },
      edit(item){
        this.$router.push({name:'bsebeadd',params:{id:item.id}});
      },
