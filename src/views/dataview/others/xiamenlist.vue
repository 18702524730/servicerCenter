<template>
  <div class="commonList clientServiceOrder">
      ...
      <div class="form_control">
        <el-input v-model="dataObj.businessName" placeholder="商标名称"></el-input>
      </div>
      <div class="form_control">
      ...
      <div class="form_control">
        <el-input v-model="dataObj.applicationNo" placeholder="申请号"></el-input>
      </div>
      ...
      <table class="result_type">
        <thead>
          <tr class="thead">
            ...
            <th class="comWidth" v-show="ziduan.indexOf(3)>-1">订单号</th>
            ...
            <th class="minWidth" v-show="ziduan.indexOf(8)>-1">通过率(%)</th>
            <th class="comWidth" v-show="ziduan.indexOf(9)>-1">服务商</th>
            <th class="comWidth" v-show="ziduan.indexOf(10)>-1">代理专员</th>

            <th class="comWidth" v-show="ziduan.indexOf(11)>-1">代理文书</th>
            <th class="comWidth" v-show="ziduan.indexOf(12)>-1">申报类别</th>
            <th class="minWidth" v-show="ziduan.indexOf(13)>-1">加项</th>
            ...
            <th class="comWidth" v-show="ziduan.indexOf(17)>-1">收款日期</th>
            <th class="comWidth" v-show="ziduan.indexOf(18)>-1">申报日期</th>
            <th class="comWidth" v-show="ziduan.indexOf(19)>-1">准予注册收文日期</th>
            <th class="comWidth" v-show="ziduan.indexOf(20)>-1">申报员</th>

            <th class="comWidth" v-show="ziduan.indexOf(21)>-1">申请号</th>
            <th class="comWidth" v-show="ziduan.indexOf(22)>-1">杭州拾贝收-代报费</th>
            <th class="comWidth" v-show="ziduan.indexOf(23)>-1">代报费收款日期</th>
            ...
            <th class="comWidth" v-show="ziduan.indexOf(27)>-1">发票收件人</th>
            <th class="comWidth" v-show="ziduan.indexOf(28)>-1">收件人电话</th>
            <th class="comWidth" v-show="ziduan.indexOf(29)>-1">收件地址</th>
            <th class="comWidth" v-show="ziduan.indexOf(30)>-1">快递单号</th>

            <th class="comWidth" v-show="ziduan.indexOf(31)>-1">不予受理</th>
            <th class="comWidth" v-show="ziduan.indexOf(32)>-1">受通是否收文</th>
            <th class="comWidth" v-show="ziduan.indexOf(33)>-1">驳回是否收文</th>
            <th class="comWidth" v-show="ziduan.indexOf(34)>-1">审查意见通知书收文日期</th>
            <th class="comWidth" v-show="ziduan.indexOf(35)>-1">初审是否收文</th>
            <th class="comWidth" v-show="ziduan.indexOf(36)>-1">初审是否上传</th>
            <th class="comWidth" v-show="ziduan.indexOf(37)>-1">是否异议答辩</th>
            <th class="comWidth" v-show="ziduan.indexOf(38)>-1">商标证是否收文</th>
            <th class="comWidth" v-show="ziduan.indexOf(39)>-1">商标证二次收文</th>
            <th class="comWidth" v-show="ziduan.indexOf(40)>-1">官文收件人</th>

            <th class="comWidth" v-show="ziduan.indexOf(41)>-1">收件人电话</th>
            <th class="comWidth" v-show="ziduan.indexOf(42)>-1">收件地址</th>
            <th class="comWidth" v-show="ziduan.indexOf(43)>-1">确认客户收件/回执</th>
            <th class="comWidth" v-show="ziduan.indexOf(44)>-1">快递单号</th>
            <th class="comWidth" v-show="ziduan.indexOf(45)>-1">寄件时间</th>
        ...
        <tbody class="tr-color">
          <tr v-for="(item, index) in dataList" v-show="!loading && dataList.length" :key="index">
            ...
            <td v-show="ziduan.indexOf(9)>-1">{{item.serviceProvider}}</td>
            <td v-show="ziduan.indexOf(10)>-1">{{item.agentCommissioner}}</td>

            <td v-show="ziduan.indexOf(11)>-1">{{item.selectDocument}}</td>
            <td v-show="ziduan.indexOf(12)>-1">{{item.reportType}}</td>
            <td v-show="ziduan.indexOf(13)>-1">{{item.addition}}</td>
            <td v-show="ziduan.indexOf(14)>-1">{{item.officialFees}}</td>
            <td v-show="ziduan.indexOf(15)>-1">{{item.serviceCharge}}</td>
            <td v-show="ziduan.indexOf(16)>-1">{{item.totalMoney}}</td>
            <td v-show="ziduan.indexOf(17)>-1">{{item.collectionDate}}</td>
            <td v-show="ziduan.indexOf(18)>-1">{{item.reportDate}}</td>
            <td v-show="ziduan.indexOf(19)>-1">{{item.permitRegisterDate | dateFormat}}</td>
            <td v-show="ziduan.indexOf(20)>-1">{{item.reporter}}</td>

            <td v-show="ziduan.indexOf(21)>-1">{{item.applicationNo}}</td>
            <td v-show="ziduan.indexOf(22)>-1">{{item.sebeCollection}}</td>
            <td v-show="ziduan.indexOf(23)>-1">{{item.agencyFeeDate | dateFormat}}</td>
            ...
            <td v-show="ziduan.indexOf(27)>-1">{{item.invoiceRevicer}}</td>
            ...
            <td v-show="ziduan.indexOf(31)>-1">{{item.dismissDate | dateFormat}}</td>
            <td v-show="ziduan.indexOf(32)>-1">{{item.acceptWhetherUpload | dateFormat}}</td>
            <td v-show="ziduan.indexOf(33)>-1">{{item.rejectWhetherReceipt}}</td>
            <td v-show="ziduan.indexOf(34)>-1">{{item.checkIdeaNoticeReceipt}}</td>
            <td v-show="ziduan.indexOf(35)>-1">{{item.firstcheckWhetherReceipt | dateFormat}}</td>
            <td v-show="ziduan.indexOf(36)>-1">{{item.firstcheckWhetherUpload}}</td>
            <td v-show="ziduan.indexOf(37)>-1">{{item.whetherDissentDefence | dateFormat}}</td>
            <td v-show="ziduan.indexOf(38)>-1">{{item.trademarkWhetherReceipt | dateFormat}}</td>
            <td v-show="ziduan.indexOf(39)>-1">{{item.trademarkWhetherSecReceipt}}</td>
            ...
            <td v-show="ziduan.indexOf(43)>-1">{{item.confirmConsumerReceive}}</td>
            <td v-show="ziduan.indexOf(44)>-1">{{item.expressNo}}</td>
            <td v-show="ziduan.indexOf(45)>-1">{{item.emailTime|dateFormat}}</td>
...
<script>
  ...
  const delUrl = './api/deleteXiamenTrademarkRegisterById';
  const listUrl = './api/selectXiamenTrademarkRegister'
  // 检查标签是否支持该属性
  ...
  {key:13,label:'加项'},
	...
	{key:17,label:'收款日期'},
	{key:18,label:'申报日期'},
	{key:19,label:'准予注册收文日期'},
  {key:20,label:'申报员'},
  
	{key:21,label:'申请号'},
  {key:22,label:'杭州拾贝收-代报费'},
  {key:23,label:'代报费收款日期'},
	{key:24,label:'纳税人识别号'},
	{key:25,label:'官费发票号'},
	{key:26,label:'服务费发票号'},
	{key:27,label:'发票收件人'},
  ...
  {key:30,label:'快递单号'},
	...
	{key:32,label:'受通是否收文'},
	{key:33,label:'驳回是否收文'},
	{key:34,label:'审查意见通知书收文日期'},
  {key:35,label:'初审是否收文'},
  {key:36,label:'初审是否上传'},
	{key:37,label:'是否异议答辩'},
	{key:38,label:'商标证是否收文'},
	{key:39,label:'商标证二次收文'},
  {key:40,label:'官文收件人'},
  
	{key:41,label:'收件人电话'},
	{key:42,label:'收件地址'},
	{key:43,label:'确认客户收件/回执'},
	{key:44,label:'快递单号'},
  {key:45,label:'寄件时间'},
  ...
  export default {
    ...
    mounted() {
      let that = this;
      let getlist = store.get('userdefaultxialist')||this.ziduan;
      this.ziduan = getlist;
    ...
    methods: {
      ...
      exportlist(){
        ...
        console.log(obj)
        window.open(this.rootUrl+'/api/exportXiamenTrademarkRegister?'+$.param(obj)+'&access_token='+this.token)
      },
      setlist(){
        store.set('userdefaultxialist',this.ziduan)
      },
      ...
      toaddrecord() {
        this.$router.push({name:'xiamenadd',params:{id:0}});
      },
      findOrderOriginAll() {
      ...
      view(item){
        this.$router.push({name:'xiamenadd',params:{id:item.id}});
      },
      edit(item){
        this.$router.push({name:'xiamenadd',params:{id:item.id}});
      },
