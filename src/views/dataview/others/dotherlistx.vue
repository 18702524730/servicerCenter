<template>
  <div class="commonList clientServiceOrder">
      ...
      <div class="form_control">
        <el-input v-model="dataObj.serviceProvider" placeholder="服务商"></el-input>
      </div>
      ...
      <table class="result_type">
        <thead>
          <tr class="thead">
            ...
            <th class="comWidth" v-show="ziduan.indexOf(5)>-1">商标名称</th>
            <th class="comWidth" v-show="ziduan.indexOf(6)>-1">业务类型</th>
            <th class="comWidth" v-show="ziduan.indexOf(7)>-1">服务商</th>
            <th class="comWidth" v-show="ziduan.indexOf(8)>-1">申报类别</th>
            <th class="comWidth" v-show="ziduan.indexOf(9)>-1">注册号</th>
            <th class="comWidth" v-show="ziduan.indexOf(10)>-1">官费</th>

            <th class="comWidth" v-show="ziduan.indexOf(11)>-1">服务费</th>
            <th class="comWidth" v-show="ziduan.indexOf(12)>-1">总金额</th>
            <th class="comWidth" v-show="ziduan.indexOf(13)>-1">财务确认收款</th>
            <th class="comWidth" v-show="ziduan.indexOf(14)>-1">收款日期</th>
            <th class="comWidth" v-show="ziduan.indexOf(15)>-1">申报日期</th>
            <th class="comWidth" v-show="ziduan.indexOf(16)>-1">申报员</th>
            <th class="comWidth" v-show="ziduan.indexOf(17)>-1">申请号</th>
            <th class="comWidth" v-show="ziduan.indexOf(18)>-1">发票抬头</th>
            <th class="comWidth" v-show="ziduan.indexOf(19)>-1">纳税人识别号</th>
            <th class="comWidth" v-show="ziduan.indexOf(20)>-1">官费发票号</th>

            <th class="comWidth" v-show="ziduan.indexOf(21)>-1">服务费发票号</th>
            <th class="comWidth" v-show="ziduan.indexOf(22)>-1">收件人</th>
            <th class="comWidth" v-show="ziduan.indexOf(23)>-1">收件电话</th>
            <th class="comWidth" v-show="ziduan.indexOf(24)>-1">收件地址</th>
            <th class="comWidth" v-show="ziduan.indexOf(25)>-1">不予受理</th>
            <th class="comWidth" v-show="ziduan.indexOf(26)>-1">收到受理通知书</th>
            <th class="comWidth" v-show="ziduan.indexOf(27)>-1">补正通知书</th>
            <th class="comWidth" v-show="ziduan.indexOf(28)>-1">驳回通知书</th>
            <th class="comWidth" v-show="ziduan.indexOf(29)>-1">更正商标申请/注册事项申请核准通知书</th>
            <th class="comWidth" v-show="ziduan.indexOf(30)>-1">初步审定公告</th>

            <th class="comWidth" v-show="ziduan.indexOf(31)>-1">核准注册</th>
            <th class="comWidth" v-show="ziduan.indexOf(32)>-1">注册证</th>
            <th class="comWidth" v-show="ziduan.indexOf(33)>-1">官文收件人</th>
            <th class="comWidth" v-show="ziduan.indexOf(34)>-1">收件人电话</th>
            <th class="comWidth" v-show="ziduan.indexOf(35)>-1">收件地址</th>
            <th class="comWidth" v-show="ziduan.indexOf(36)>-1">确认客户收件/回执</th>
            <th class="comWidth" v-show="ziduan.indexOf(37)>-1">快递单号</th>
            <th class="comWidth" v-show="ziduan.indexOf(38)>-1">寄件时间</th>
            <th class="comWidth" v-show="ziduan.indexOf(39)>-1">邮寄方式</th>
        ...
        <tbody class="tr-color">
          <tr v-for="(item, index) in dataList" v-show="!loading && dataList.length" :key="index">
            ...
            <td v-show="ziduan.indexOf(5)>-1">{{item.trademarkName}}</td>
            <td v-show="ziduan.indexOf(6)>-1">{{item.businessType}}</td>
            <td v-show="ziduan.indexOf(7)>-1">{{item.serviceProvider}}</td>
            <td v-show="ziduan.indexOf(8)>-1">{{item.declareType}}</td>
            <td v-show="ziduan.indexOf(9)>-1">{{item.registerNo}}</td>
            <td v-show="ziduan.indexOf(10)>-1">{{item.officialFees}}</td>

            <td v-show="ziduan.indexOf(11)>-1">{{item.serviceCharge}}</td>
            <td v-show="ziduan.indexOf(12)>-1">{{item.totalMoney}}</td>
            <td v-show="ziduan.indexOf(13)>-1">{{item.financialConfirmCollection}}</td>
            <td v-show="ziduan.indexOf(14)>-1">{{item.collectionDate}}</td>
            <td v-show="ziduan.indexOf(15)>-1">{{item.reportDate|dateFormat}}</td>
            <td v-show="ziduan.indexOf(16)>-1">{{item.reporter}}</td>
            <td v-show="ziduan.indexOf(17)>-1">{{item.applicationNo}}</td>
            <td v-show="ziduan.indexOf(18)>-1">{{item.invoiceTitle}}</td>
            <td v-show="ziduan.indexOf(19)>-1">{{item.taxpayerNumber}}</td>
            <td v-show="ziduan.indexOf(20)>-1">{{item.officialFeesInvoiceNumber}}</td>
            
            <td v-show="ziduan.indexOf(21)>-1">{{item.serviceChargeInvoiceNumber}}</td>
            <td v-show="ziduan.indexOf(22)>-1">{{item.recevier}}</td>
            <td v-show="ziduan.indexOf(23)>-1">{{item.receviePhone}}</td>
            <td v-show="ziduan.indexOf(24)>-1">{{item.recevieAddress}}</td>
            <td v-show="ziduan.indexOf(25)>-1">{{item.dismissNotice}}</td>
            <td v-show="ziduan.indexOf(26)>-1">{{item.getAcceptNotice}}</td>
            <td v-show="ziduan.indexOf(27)>-1">{{item.updateNotice}}</td>
            <td v-show="ziduan.indexOf(28)>-1">{{item.rejectNotice}}</td>
            <td v-show="ziduan.indexOf(29)>-1">{{item.updateTrademarkApply}}</td>
            <td v-show="ziduan.indexOf(30)>-1">{{item.preCheckNitoce}}</td>

            <td v-show="ziduan.indexOf(31)>-1">{{item.checkRegister}}</td>
            <td v-show="ziduan.indexOf(32)>-1">{{item.registration}}</td>
            <td v-show="ziduan.indexOf(33)>-1">{{item.officialRecevier}}</td>
            <td v-show="ziduan.indexOf(34)>-1">{{item.officialRecevierPhone}}</td>
            <td v-show="ziduan.indexOf(35)>-1">{{item.officialRecevierAddress}}</td>
            <td v-show="ziduan.indexOf(36)>-1">{{item.confirmConsumerReceive}}</td>
            <td v-show="ziduan.indexOf(37)>-1">{{item.expressNumber}}</td>
            <td v-show="ziduan.indexOf(38)>-1">{{item.expressTime}}</td>
            <td v-show="ziduan.indexOf(39)>-1">{{item.emailType}}</td>
...
<script>
  ...
  const delUrl = './api/deleteOtherbusinessylById';
  const listUrl = './api/selectOtherbusinessyl'
  // 检查标签是否支持该属性
  ...
  {key:20,label:'官费发票号'},
	...
	{key:24,label:'收件地址'},
	{key:35,label:'不予受理'},
	{key:26,label:'收到受理通知书'},
  {key:27,label:'补正通知书'},
	{key:28,label:'驳回通知书'},
	{key:29,label:'更正商标申请/注册事项申请核准通知书'},
  {key:30,label:'初步审定公告'},
  ...
  export default {
    ...
    mounted() {
      let that = this;
      let getlist = store.get('userdefaultdotherxlist')||this.ziduan;
      this.ziduan = getlist;
    ...
    methods: {
      ...
      exportlist(){
        ...
        console.log(obj)
        window.open(this.rootUrl+'/api/exportOtherbusinessyl?'+$.param(obj)+'&access_token='+this.token)
      },
      setlist(){
        store.set('userdefaultdotherxlist',this.ziduan)
      },
      ...
      toaddrecord() {
        this.$router.push({name:'dotheraddx',params:{id:0}});
      },
      findOrderOriginAll() {
      ...
      view(item){
        this.$router.push({name:'dotheraddx',params:{id:item.id}});
      },
      edit(item){
        this.$router.push({name:'dotheraddx',params:{id:item.id}});
      },
