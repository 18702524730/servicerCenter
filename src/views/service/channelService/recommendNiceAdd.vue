<template>
	<div class="steptwo_fill_nice" v-loading="loading" element-loading-text="拼命加载中">
		...
		<div class="nisibox">
            <div class="formbox">
                <el-form :model="dataObj" :inline="true"  :rules="rules" ref="ruleForm" label-width="100px">
                    <el-form-item label="订单编号：" prop="orderSn">
                        <el-input type="text" v-model="dataObj.orderSn" autocomplete="off"></el-input>
                    </el-form-item>
                    <el-form-item label="旺旺号：" prop="wangwangSn">
                        <el-input type="text" v-model="dataObj.wangwangSn" autocomplete="off"></el-input>
                    </el-form-item>
                    
                    <el-form-item label="商标名称：" prop="trademarkName">
                        <el-input type="text" v-model="dataObj.trademarkName" autocomplete="off"></el-input>
                    </el-form-item>
                    <el-form-item label="下单时间：" prop="orderTime">
                        <el-date-picker v-model="dataObj.orderTime" type="datetime"></el-date-picker>
                    </el-form-item>
                    <el-form-item label="商标图样：">
						<div class="upload_component">
							<uploader @complete="setUploadedData" @resetUploader="resetUploader" :fileName="standby_name" :postAction="'iprp_servicer/api/file/uploadPic'" :url="standby" :valueName="'standby'" :size="0.2" :extensions="'jpg,jpeg'"></uploader>
    ...
    <div class="fill_states" v-if="draftInfo.length && draftInfo[0].class_id&&!loading">
    	...
    	<div class="state" v-for="(info,$draftIndex) in draftInfo" :key="$draftIndex">
          ...
          <!-- 收缩 -->
    			<img src="~assets/img/shrinkage.png" alt="" v-if="!info.PackUp" @click.stop="closeCurOrder(info)">
          ...
          <!-- 收缩后显示的内容 -->
					<ul class="mt10 clearfix"  v-if="info.PackUp && hasSomething(info)">
						...
						<li class="clearfix mb10">
							...
							<p class="state_txt fl">
								{{info.niceGroupGovernNums}}
							</p>
						</li>
					</ul>

					<div class="add mt20" v-if="$draftIndex === draftIndex&&!info.PackUp" @click.stop="">
						<p>请选择商标小项：</p>
                ...
                <!-- 选中名称 -->
                    ...
                    <!-- 小项 -->
				                ...
				                <p slot="content" class="smallFee">已超出{{curSelectedSmallList.length-10}}个小项<br>追加费用27x{{curSelectedSmallList.length-10}}元</p>
											  <span class="cy">￥{{curSelectedSmallList.length && curSelectedSmallList.length>10 ? (curSelectedSmallList.length-10)*27 : '0'}}.00</span>
				              </el-tooltip>
										</p>
									</div> -->
								</div>
					...
					<a href="#" class="del" :class="{disabled: draftInfo && draftInfo.length == 1}" @click.prevent.stop="askDelete($draftIndex)">删除</a>
				</div>
    	</div>
	    <div class="confirm_btn clearfix" v-if="!loading">
	    	<button class="creat_url" @click="checkform">生成小项推荐链接</button>
			</div>
    ...
    <div class="draft_nulls" v-if="!isedit&&(!draftInfo.length || !draftInfo[0].class_id) && !loading ">
...
<script>
  ...
  import qs from 'qs'
	...
	// import myCropper from 'cps/uploader/cropper.vue'
  ...
  const initdata ={
    ...
    data() {
      return {
		  dataObj: Object.assign({},initdata),
		  isedit: false,
          standby:'',
          standby_name:'',
          rules:{
              orderSn:[{required: true,message:'请输入订单编号',trigger:'blur'}],
              wangwangSn:[{required: true,message:'请输入旺旺号',trigger:'blur'}],
    ...
    methods: {
		...
		getDetail(){
			...
			console.log(key)
			// 
			this.$http.get('./api/findChannelNiceRec?key='+key)
			.then((resp)=>{
				console.log(resp.data,'监测')
				this.draftInfo = resp.data.work_order_service;
				// this.niceMinPrice = resp.data.niceMinPrice;
				this.dataObj = resp.data
				this.standby = this.dataObj.trademarkUrl
				if(this.draftInfo){
				...
				this.userCenter=userUrl+'/iprp/#/TmallRecommendNiceLink?key='+key
			}).catch(err=>{
      ...
      getMatchMin(str){
    	...
    	makeUrl(){
			...
			let queryobj= Object.assign({},this.dataObj)
			queryobj.orderSn=this.trimstr(this.dataObj.orderSn)
			queryobj.wangwangSn=this.trimstr(this.dataObj.wangwangSn)
			queryobj.trademarkName=this.trimstr(this.dataObj.trademarkName)
            queryobj.trademarkUrl = this.standby;
            let stamp = new Date(this.dataObj.orderTime).getTime()
			queryobj.orderTime = stamp
            ...
            let rest = Object.assign({},data,queryobj);
    		this.$http.post('./api/createChannelRecLink',rest)
    		.then((resp)=>{
				this.userCenter=userUrl+'/iprp/#/TmallRecommendNiceLink'+resp.data
				this.key = resp.data.replace('?key=','')
				this.copyTxt = "复制"
				this.loading = false;
...
.nisibox{
    ...
    .msgColor{color: #F64744 !important;}
		...
		.fill_states{padding:30px 90px;background-color:#fff;min-height: 500px;border:1px solid #DFE2E5;border-top:none;
      ...
      div.state_list:hover{background: #F9F9F9;box-shadow: 0 1px 4px 0 rgba(0,0,0,0.08);}
		...
		.draft_nulls{min-height:500px;background-color:#fff;text-align: center;border:1px solid #DFE2E5;border-top:none;
