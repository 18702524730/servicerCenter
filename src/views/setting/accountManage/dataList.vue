<template>
  <div class="commonList dataList">
	  <div class="result">
	  	<div class="btn_action">
	  		<button class="add" @click="addData">+新增组</button>
	  	</div>
        ...
        <tbody class="tr-color">
          <tr v-for="(item, index) in dataList">
            ...
            <td class="action">
            	<a href="javascript:;" class="view" @click="editData(item)">编辑</a>
            </td>
          </tr>
        </tbody>
	    </table>
...
<script>
	const dataListUrl = './api/dataGroups';
 import Filters from 'utils/filters/'
  export default {
    ...
    methods: {
      view(item){
        this.$router.push({name: 'accountDetail', params: {id: item.dg_id}})
      },
      ...
      //增加账号
      addData(){
      	this.$router.push({name: 'dataEdit', params: {operator_id: 0}});
      },
      //编辑账号
      editData(item){
      	this.$router.push({name: 'dataEdit', params: {operator_id: item.dg_id}});
      },
    },
    '$route' (to, from) {
