<template>
	<div class="step2_fill_ch" v-loading="loading" element-loading-text="拼命加载中">
		...
		<el-form class="basic_info" ref="tmForm" :model="trademarkData" :rules="tmFormRules" :inline="true" auto-complete="off" label-width="120px" v-if="!loading">
			<el-form-item class="addAddress" label="商标类型：" prop="trademark_type" style="width:53%" required >
				<template>
						...
						<el-radio class="radio" v-model="trademarkData.trademark_type"  :label="3">图文组合商标</el-radio>
				</template>
			</el-form-item>

			<el-form-item class="addAddress" label="商标名称：" prop="trademark_name" style="width:53%">
				<el-input size="small" v-model="trademarkData.trademark_name" placeholder="请输入商标名称" :maxlength="50"></el-input>
			</el-form-item>
          ...
          <myCropper :option="optionSpecify" :size="0.2" :extensionsType="1" :valueName="'standby2'" :url="standby2" :uploadedFileData="uploadedFileDataSpecify" :minWidth="400" :minHeight="400" :maxWidth="1500" :maxHeight="1500" :postAction="'/api/file/uploadPic'" @resetUploader="resetUploader"  @showStatus="showStatus2Fn" @submitFn="setUploadedDataSpecify" @update="update2Fn" v-if="!isIE"></myCropper>
					<div class="upload_tip">
						...
						<!-- <p style="padding-top:8px;"><a target="_blank" :href="trademarkData.specify_pic_url"><img src="~assets/img/action-icon.png" alt="">下载客户提交的商标图样</a></p> -->
					</div>
				</div>
          ...
          <myCropper :option="optionBlack" :size="0.2" :extensionsType="1" :valueName="'standby1'" :url="standby1" :uploadedFileData="uploadedFileDataBlack" :minWidth="400" :minHeight="400" :maxWidth="1500" :maxHeight="1500" :postAction="'/api/file/uploadPic'" @resetUploader="resetUploader"  @showStatus="showStatus0Fn" @submitFn="setUploadedDataBlack" @update="update0Fn" v-if="!isIE"></myCropper>
					<div class="upload_tip">
						<p v-if="trademarkData.pic_type==1" class="autocreate"><a href="javascript:void 0" @click.prevent="autocreatepic">自动生成</a></p>
						<p>黑白色图片<span v-if="trademarkData.pic_type == 2">(可选填)</span></p>
						<p class="format">格式说明：格式为jpg，图形应清晰，图样文件大小应小于200KB且图形像素介于“400×400”－“1500×1500”之间。</p>
						<!-- <p v-if="trademarkData.pic_type==1" style="padding-top:8px;"><a target="_blank" :href="trademarkData.black_white_pic_url"><img src="~assets/img/action-icon.png" alt="">下载客户提交的商标图样</a></p> -->
					</div>
				</div>
			</el-form-item>
          ...
          <myCropper :option="optionProxy" :size="2" :extensionsType="1" :valueName="'standby3'" :url="standby3" :uploadedFileData="uploadedFileDataProxy" :minWidth="600" :minHeight="600" :maxWidth="4000" :maxHeight="4000" :postAction="'/api/file/uploadProxy'" @resetUploader="resetUploader"  @showStatus="showStatusFn" @submitFn="setUploadedDataProxy" @update="updateFn" v-if="!isIE"></myCropper>
					<div class="upload_tip">
						...
						<p style="padding-top:8px;"><a class="downloadbtn" href="javascript:void 0" @click.prevent="download"><img src="~assets/img/action-icon.png" alt="">下载委托书模板</a></p>
					</div>
				</div>
			</el-form-item>

			<el-form-item label="上传合同书：" style="width:80%">
				<div class="upload_component" style="margin-bottom:20px;">
					...
					<div class="upload_tip">
						...
						<p class="delfile" v-if="standby4" @click="resetUploader('standby4')">删除文件</p>
						<p class="format">系统已上传客户提交的合同书。若需修改，请上传修正资料。<br>格式说明：格式为RAR、ZIP、PDF、JPG、BMP、PNG，大小限5M以内；</p>
						<p style="padding-top:8px;"><a href="javascript:void 0" class="downloadbtn" @click.prevent="nextStep(false,downloadHtFn)"><img src="~assets/img/action-icon.png" alt="">下载合同书模板</a></p>
					</div>
				</div>
			</el-form-item>

			<el-form-item label="上传商标确认书：" style="width:80%">
				<div class="upload_component" style="margin-bottom:20px;">
					...
					<div class="upload_tip">
						...
						<p class="delfile" v-if="standby5"  @click="resetUploader('standby5')">删除文件</p>
						<p class="format">系统已上传客户提交的商标确认书。若需修改，请上传修正提交。格式说明：格式为RAR、ZIP、PDF、JPG、BMP、PNG，大小限5M以内；</p>
						<p style="padding-top:8px;"><a href="javascript:void 0" class="downloadbtn" @click.prevent="nextStep(false,downloadSureFn)"><img src="~assets/img/action-icon.png" alt="">下载商标确认书模板</a></p>
					</div>
				</div>
			</el-form-item>
		...
		<div class="sometips" v-if="!loading">合同书和商标确认书必须上传一个</div>

		<div class="fill_state" v-if="!loading">
			<p class="mb20">商标分类信息</p>
			<div class="tm_info_btn">
    		<button class="creat_url" @click="bigClassVisible=true"><span>+</span>增加大类</button>
    	</div>
			<div class="state" v-for="(info,$draftIndex) in draftInfo" :key="$draftIndex">
				<div class="state_list" :class="{'bgc':$draftIndex === draftIndex}">
					...
					<div class="add mt20" v-if="$draftIndex === draftIndex&&!info.PackUp">
						...
						<ul class="clearfix mt15">
							<li v-for="(item,$index) in bigState" :class="{'click_li':$index+1 == index, 'notChoose': allSelectedBigData.indexOf($index+1) != -1 && $index+1 != index}" class="fl mr10 mb10" @click="selected(item, info)" :key="$index">{{item.class_name}}</li>
						</ul>
							...
							<div class="events_left fl">
								...
								<!-- 选中名称 -->
									...
									<ul class="fuzzyqueryList" v-if="FuzzyQueryData.length">
										<li v-for="item in FuzzyQueryData" @click="FuzzyQueryClick(item.min_id)" v-html="item.min_name" :key="item.min_id"></li>
									</ul>
								...
								<div class="list_over" v-loading="smallLoading">
									<div class="events_list" v-for="(item,$index) in niceEventsData" :key="$index">
										<!-- 小项头部 -->
								...
								<div class="right_num clearfix">
									...
									<p class="right_txt fl">已选<span class="cy">{{curSelectedSmallList.length}}</span>项</p>
									<p class="right_btn fr" @click="resetEvent">重置</p>
								</div>
								<div class="right_list">
									<ul>
										<li v-for="events in curSelectedSmallList" style="text-align:left" class="mb10" :key="events.min_id">
						...
						<!-- <div class="footer_inform">
							...
							<div class="shrink" v-if="!info.PackUp" @click="closeCurOrder(info)">确定</div>
						</div> -->
					</div>
		...
		<div class="confirm_btn clearfix" v-if="!loading">
			...
			<p class="fl btn_bg" @click="nextStep(true)">下一步</p>
		</div>
		<el-dialog title="提示"
...
<script>
	...
	import myCropper from 'cps/uploader/movecropper.vue';  //引入封装好的裁切组件
	const rootUrl = CONFIG.rootUrl;
	const niceUrl= './api/getNiceClass';
	const niceEventsUrl = './api/getNiceGroupByClassId';
	const InfoUrl = './api/channelWorkOrderQuery';
	const draftUrl = './api/saveChannelWorkOrder';
	const submitUrl = './api/submit';
	const autocreateUrl = './api/autoCreateTmPic'// 自动生成商标图样
	...
	export default {
		...
		methods: {
			...
			setUploadedDataBlack(data){
				...
				if (d.url) {
					...
					this.trademarkData.black_white_pic_file_name = d.fileName;
				}else{
			...
			setUploadedDataSpecify(data){
				...
				if (d.url) {
					...
					this.trademarkData.specify_pic_fileName = d.fileName;
				}else{
					this.uploaderErrCallback(d);
				}
			},
			setUploadedDataProxy(data){
				...
				if (d.url) {
					...
					this.trademarkData.proxy_file_name = d.fileName;
				}else{
					this.uploaderErrCallback(d);
				}
			},
			setUploadedContract(data){
				...
				if (d.url) {
					...
					this.trademarkData.contract_file_name = d.fileName;
				}else{
					this.uploaderErrCallback(d);
				}
			},
			setUploadedConfirm(data){
				...
				if (d.url) {
					...
					this.trademarkData.confirm_file_name = d.fileName;
				}else{
					this.uploaderErrCallback(d);
				}
			},
			...
			getinfo(){
				...
				.then(function(resp){
					self.loading = false;
					for(var i=0;i<resp.data.workOrderServiceList.length;i++){
						resp.data.workOrderServiceList[i].PackUp = true;
					}
    			self.niceMinPrice = resp.data.niceMinPrice;
					...
					if(self.trademarkData.contract_file_name){
						self.standby4_name = self.trademarkData.contract_file_name;
					}
					if(self.trademarkData.confirm_file_name){
						self.standby5_name = self.trademarkData.confirm_file_name;
					}
					

					//服务单部分
					self.draftInfo = resp.data.workOrderServiceList;

					self.workOrderId = resp.data.workOrderId;
			...
			saveDraft(isNext,fn){
				...
				// self.allEvents.workOrderServiceList = self.draftInfo;
				self.allEvents.workOrderServiceList = arr;
				self.allEvents.workOrderSn = self.workOrderSn;
				self.allEvents.confirmStep = 2;
				// 保存草稿接口
				...
				.then((resp) => {
					...
					if (!isNext) {
						...
						self.$router.push({name: 'stepThreeCh', query: Object.assign({}, query, {confirmStep: 3})});
					}
				})
				.catch((err) => {
					if (err && err.msg) {
			...
			openCurOrder(orderindex, info){
				...
				console.log(self.curSelectedSmallList)
				//如果有大类，则查询小项
				if (info.class_id) {
			...
			checkAllOrders(){
				...
				for(var i=0;i<draftInfo.length;i++){
					...
					if(!info.class_id || !info.service_nice_min.length ){
						self.$message({
							message: '请完善商标注册信息',
							type: 'warning'
						});
			...
			nextStep(isnext,fn){
				this.$refs['tmForm'].validate((valid) => {
					if (valid) {
						if(isnext){
							if(this.standby4||this.standby5){
								if(this.checkUploadFile() && this.checkAllOrders()){
									this.saveDraft(isnext,fn);
								}
							}else{
								this.$message.warning('合同书和商标确认书必须上传一个')
							}
						}else{
			...
			prev(){
				...
				delete query.link
				this.$router.push({name: 'stepOneCh', query: Object.assign({}, query, {confirmStep: 1})})
			},
			// 模糊查询
			DataFuzzyQuery(data){
				var self = this;
			...
			autocreatepic(){
				this.$refs['tmForm'].validate((valid) => {
					if(valid && !this.isrepeat){
						
						this.isrepeat = true;
						this.$http.get(autocreateUrl, {params :{
							trademarkName: this.trademarkData.trademark_name,
						}})
						.then((resp) => {
							this.isrepeat = false;
							var data = resp.data;
							if (data.url) {
								...
								this.trademarkData.black_white_pic_url = data.url
								this.standby1_name = data.fileName;
								this.trademarkData.black_white_pic_name = data.fileName
			...
			downloadHtFn () {
				this.$http.get(downContractTemplateUrl, {params :{
					work_order_sn: this.workOrderSn,
				}})
				.then((resp) => {
					var data = resp.data;
					if (data.url) {
			...
			queryDetailnice(){
				...
				.then((resp) => {
					this.loading = false;
					var data = resp.data;
					this.trademarkData.trademark_name = data.trademarkName;
					this.standby1 = data.trademarkUrl;
					this.trademarkData.black_white_pic_url = data.trademarkUrl;
					let worklist = data.work_order_service;
					if(worklist){
						this.draftInfo = worklist;
						if(this.draftInfo){
		...
		mounted() {
			...
			this.workOrderId = this.$route.query.workOrderId||null
			this.link = this.$route.query.link||null
			// let ceshidata = store.get('ceshidata');
			// this.draftInfo = ceshidata;

			// 	if(this.draftInfo){
			// 		this.draftInfo.forEach(item=>{
			// 			item.PackUp = true;
			// 			// this.checkedBigList.push(item.class_id);
...
<style lang='less'>
	.step2_fill_ch{padding:60px 60px;background-color:#fff;min-height: 500px;
		...
		.upload_component{position: relative;
			...
			.autocreate a{
				display:inline-block;padding: 0 15px;height:24px;line-height:24px; border-radius: 12px;background-color: #379dea; color:#fff;text-decoration: none;margin-bottom: 10px;
			}
		}
		...
		.fill_state{
			...
			div.state_list{padding:22px 0 20px;position: relative;
				...
				.footer_inform{
					...
					.shrink{width: 80px;margin:0 auto;height: 30px;border: 1px solid #479CFF;border-radius: 2px;color: #479CFF;line-height: 28px;text-align: center;cursor: pointer;}
				}
				}
			}
			.del{
