    ...
    addInputFile(el) {
      ...
      if (el.files) {
        for (let i = 0; i < el.files.length; i++) {
          let file = el.files[i]
          // 判断图片宽高
          if(file.type.indexOf('png')!=-1 || file.type.indexOf('jpeg')!=-1 ||file.type.indexOf('jpg')!=-1 || file.type.indexOf('bmp')!=-1){
            ...
            img.onload = function () {
              ...
              if (this.width<self.minWidth || this.height<self.minHeight||this.width>self.maxWidth || this.height>self.maxHeight) {
                // self.$alert(`图形像素必须介于“${self.minWidth}×${self.minHeight}”－“${self.maxWidth}×${self.maxHeight}”之间`);
                // return Promise.reject(new Error('chaochu'))
                self.add({
                  size: file.size,
                  name: file.webkitRelativePath || file.name,
                  type: file.type,
